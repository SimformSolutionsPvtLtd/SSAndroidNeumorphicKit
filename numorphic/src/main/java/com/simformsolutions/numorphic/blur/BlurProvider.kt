package com.simformsolutions.numorphic.blur
...
internal class BlurProvider(context: Context) {
     ...
     * @param radius Radius to be applied on bitmap.
     * @param sampling
     *
    ...
    private fun rs(bitmap: Bitmap, radius: Int): Bitmap? {
        ...
        try {
            ...
            /** Destroy everything to handle memory leak. */
            rs?.destroy()
            input?.destroy()
            output?.destroy()
            blur?.destroy()
    ...
    /**
     * In case [rs] method failed to blur will use StackBlur to apply blur effect.
     */
